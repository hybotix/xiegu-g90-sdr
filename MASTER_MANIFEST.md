...
| File                 | Type | Purpose                                                   |
|----------------------|------|-----------------------------------------------------------|
| `g90_sdr.yaml`       | YAML | Main system configuration (FlRig, SDR++, Audio, Sync)    |
...
| `frequency_sync.py` | Python | ~350   | Real-time frequency synchronization (G90 ↔ SDR++)      |
...
| File                | Type  | Lines* | Purpose                                              |
|---------------------|-------|--------|------------------------------------------------------|
| `build_flrig.bash`  | Shell | ~200   | Build FlRig from source (auto-detects latest)       |
| `build_sdrpp.bash`  | Shell | ~250   | Build SDR++ from source (auto-detects latest)       |
| `rebuild_all.bash`  | Shell | ~100   | Rebuild all components to latest versions           |
...
## 🔧 System-Wide Files (Not in Repository)
...
| `sdrpp`     | Binary | SDR++ application (built from source)               |
...
| `sdrpp-g90.desktop`    | Desktop | SDR++ launcher                          |
...
- utils/build_flrig.bash
- utils/build_sdrpp.bash
...
```
start_sdr.py
├── requires: rig_control.py
├── requires: frequency_sync.py
├── requires: FlRig (system)
├── requires: SDR++ (system)
└── requires: venv/

install.bash
├── creates: venv/
├── builds: FlRig (to /usr/local/bin/)
├── builds: SDR++ (to /usr/local/bin/)
└── installs: Python packages

install_wrappers.bash
├── creates: /usr/local/bin/g90-sdr
└── creates: /usr/local/bin/g90-flrig
```
...
### Configuration
- Template files for FlRig, audio routing
...
1. ✅ **install.bash** - Without this, installation impossible
2. ✅ **requirements.txt** - Python dependencies
3. ✅ **scripts/start_sdr.py** - Main entry point
4. ✅ **scripts/rig_control.py** - FlRig communication
5. ✅ **scripts/frequency_sync.py** - Core synchronization
6. ✅ **config/g90_sdr.yaml** - System configuration
7. ✅ **install_wrappers.bash** - System integration
...
[ -f INSTALL.md ] && echo "✓ INSTALL.md"
[ -f install.bash ] && echo "✓ install.bash"
[ -f requirements.txt ] && echo "✓ requirements.txt"

# Core scripts
[ -f scripts/start_sdr.py ] && echo "✓ start_sdr.py"
[ -f scripts/rig_control.py ] && echo "✓ rig_control.py"
[ -f scripts/frequency_sync.py ] && echo "✓ frequency_sync.py"

# Test scripts
[ -f tests/TestConnection.py ] && echo "✓ TestConnection.py"
[ -f tests/DiagnoseSystem.py ] && echo "✓ DiagnoseSystem.py"

# Utils
[ -f utils/rebuild_all.bash ] && echo "✓ rebuild_all.bash"
